...
setupfirewall() {
    ...
    case "$(readlink -f /sbin/init)" in
        *systemd*)
            # SYSTEMD-ABHÄNGIGKEIT: systemctl wird verwendet um den Dienst zu aktivieren
            systemctl enable ufw >/dev/null 2>&1
            ;;
        *runit*)
            # RUNIT-ABHÄNGIGKEIT: Symlink des runit-Service in das run-Verzeichnis
...
# SYSTEMD/ELOGIND-ABHÄNGIGKEIT: loginctl gehört zu systemd (oder elogind bei anderen Init-Systemen).
# Wenn elogind installiert ist, funktioniert dies auch unter runit.
if command -v loginctl >/dev/null 2>&1; then
    loginctl enable-linger "$name"
else
    whiptail --infobox "loginctl nicht gefunden (kein systemd/elogind). Linger konnte nicht aktiviert werden." 7 60
    sleep 3
fi
...
if [ -d /sys/class/bluetooth ] && [ -n "$(ls -A /sys/class/bluetooth 2>/dev/null)" ]; then
    ...
    case "$(readlink -f /sbin/init)" in
        *systemd*)
            ...
            ;;
        *runit*)
            # RUNIT-ABHÄNGIGKEIT: Bluetooth Dienst für runit aktivieren
...
# SYSTEMD-ABHÄNGIGKEIT: user-systemctl existiert unter runit nicht in dieser Form.
# RUNIT: Unter runit werden Benutzerdienste oft über ~/.xinitrc oder dinit/turnstile/runit-user gestartet.
case "$(readlink -f /sbin/init)" in
    *systemd*)
        ...
        ;;
    *runit*)
        # RUNIT-ABHÄNGIGKEIT: Hinweis ausgeben, da systemctl --user nicht nativ ist.
        # Dienste wie Pipewire müssen meist im Window Manager Autostart (.xinitrc oder aehnliches) gestartet werden.
        whiptail --infobox "Unter runit muss PipeWire typischerweise in der ~/.xinitrc gestartet werden." 7 60
        sleep 2
        ;;
esac
...
# SYSTEMD-ABHÄNGIGKEIT: Nur unter systemd muss der Service maskiert werden.
if [ "$(readlink -f /sbin/init | grep -c systemd)" -gt 0 ]; then
    sudo -u "$name" XDG_RUNTIME_DIR="/run/user/$(id -u "$name")" \
...
# SYSTEMD-ABHÄNGIGKEIT: systemctl Befehle hier sind systemd spezifisch,
# aber unter runit werden oft zzz/ZZZ (void-tools) oder loginctl (elogind) genutzt. Wir fügen beides hinzu.
