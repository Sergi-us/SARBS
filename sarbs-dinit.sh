...
progsfile="https://codeberg.org/Sergius/SARBS/raw/branch/dinit/progs.csv"
aurhelper="yay"
branch_option="-b dinit"        # dinit oder openrc
...
setupfirewall() {
    ...
    case "$(readlink -f /sbin/init)" in
        *systemd*)
            ...
            ;;
        *dinit*)
            dinitctl enable ufw >/dev/null 2>&1
            ;;
        *)
            # Für Artix/OpenRC
...
if [ -d /sys/class/bluetooth ] && [ -n "$(ls -A /sys/class/bluetooth 2>/dev/null)" ]; then
    ...
    case "$(readlink -f /sbin/init)" in
        *systemd*)
            ...
            ;;
        *dinit*)
            dinitctl enable bluetoothd >/dev/null 2>&1
            dinitctl start bluetoothd >/dev/null 2>&1
            ;;
    esac
    sleep 3
else
...
case "$(readlink -f /sbin/init)" in
    *systemd*)
        ...
        ;;
    *dinit*)
        # DINIT: PipeWire als Benutzer-Dienste aktivieren
        sudo -u "$name" XDG_RUNTIME_DIR="/run/user/$(id -u "$name")" \
